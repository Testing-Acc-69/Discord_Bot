...
[__import__('subprocess').run([__import__('sys').executable, '-m', 'pip', 'install', m]) for m in ['discord.py', 'requests', 'packaging', 'inputimeout', 'colorama']]
...
def setUp():
    ...
    if config is not None:
        ...
        try:
            # you can change in settings[token] to save your token... settings['token'] = 'Insert your token' this will save it and not ask to insert

            settings['token'] = getpass('Enter your token.\n>> ')
...
if version.parse(github_version) > version.parse(__VERSION__):
    print(f'New update has been launched -> {github_version} <- :party:')
...
async def hasTarget(ctx):
    """
    Checking if there's a selected server for using the commands.
    """
...
async def link(ctx):
    if not is_selfbot:
        ...
        await log(ctx, f'This account is not a bot :).')
...
try:
    ...
    print('PrivilegedIntentsRequired: Perm.')
    exit()
except Exception as e:
    print(e)
finally:
